...
if(empty($_REQUEST['DR_NO'])){
  ?><script>window.location="../dr_list/";</script><?php
  
}
if(isset($_REQUEST['DR_NO'])){
  $fetch = mysqli_query($link,"select * from dr inner join customer ON dr.CUSTOMER_ID = customer.CUSTOMER_ID where DR_NO=".$_REQUEST['DR_NO']);
  if(mysqli_num_rows($fetch)>0){
    $catch = mysqli_fetch_array($fetch);
    $CUSTOMER_NAME = $catch['CUSTOMER_NAME'];
      $CUSTOMER_ID = $catch['CUSTOMER_ID'];
    $CYLINDER_ID = $catch['CYLINDER_REFERENCEID'];
  $DR_STARTDATE = $catch['DR_STARTDATE'];
   $DR_RETURNDATE = $catch['DR_RETURNDATE'];
 $DR_NO = $catch['DR_NO'];
 
  }else{
      ?><script>alert("sorry, but we dont have that id.");window.location="../dr_list/";</script><?php
...
<?php
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <h3 class="box-title"><i class="fa fa-plus text-danger">&nbsp;</i>Edit Direct Receipt</h3>
         <a class="btn btn-primary btn-sm pull-right" href="add_more/index.php?DR_NO=<?php echo $DR_NO;?>"><span class="fa fa-plus"></span>&nbsp;<strong>Add more data to this DR number</strong></a>
    ...
    <span class="label label-primary">DR Number : <?php echo $DR_NO;?></span><br><br>
         <div class="row">
         <input type="hidden" min="0" name="DR_NO" value="<?php echo $DR_NO;?>" class="form-control pull-right" id="DR_NO" required/>
         <div class="col-md-6">
          <label>Select DR Number</label>
         <input type="number" min="0" name="DR_NO_ORIG" value="<?php echo $DR_NO;?>" class="form-control pull-right" id="DR_NO" required/>
        </div>
        <div class="col-md-6">
            ...
            <div class="form-group">
                ...
                <div class="input-group date">
                  ...
                  <input type="hidden" name= "DR_ID" value="<?php echo $DR_ID;?>"></input>
                  <input type="text" name="DR_STARTDATE" value="<?php echo $DR_STARTDATE;?>" class="form-control pull-right" id="datepicker" required/>
                </div>
                <!-- /.input group -->
              </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="input-group date">
                  ...
                  <input type="text" name="DR_RETURNDATE" value="<?php echo $DR_RETURNDATE;?>" class="form-control pull-right" id="datepicker2" required />
                </div>
                <!-- /.input group -->
              </div>
            </div>

...
if(isset($_REQUEST['delete_id']))
{
  ...
  $audit_query = mysqli_query($link,"insert into audittrail(AUDITTRAIL_ACTIVITY,AUDITTRAIL_ACTION,AUDITTRAIL_INFO,AUDITTRAIL_DATE,AUDITTRAIL_DATEONLY,AUDITTRAIL_USER) VALUES('IR Module','Deleted inward reciept','$info $ir_no $info2 $cyl_id','$IR_DATEDELETED','$IR_DATEONLY','$userid')");
if($audit_query){
 $sql_query="Update ir set IR_STATUS='inactive',IR_DATEDELETED='$IR_DATEDELETED' WHERE IR_ID='$request_id'";
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title"><span class="fa fa-edit"></span>&nbsp;Please enter the IR Number you want to modify</h4>
      </div>
      <div class="modal-body">
      <form action="../ir_edit/" action="POST"> 
      <div class="form-group">
                <label>Select IR Number</label>
                  <?php $query = "select DISTINCT(IR_NO) from ir where IR_STATUS =''";
                $result2 = mysqli_query($link,$query);
                ?>
                <select class="form-control select2"  name="IR_NO" data-placeholder="Select IR Number" style="width: 100%;" required>
                 <option label="-- Select IR Number--" ></option>
                  <?php while($row2 = mysqli_fetch_array($result2)){
             
               $IR_NO = $row2['IR_NO'];
                ?> <option value="<?php echo $IR_NO;?>"><?php echo $IR_NO;?></option><?php
                }?>
...
<!--######################################## MODAL ###############################################################-->
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title"><span class="fa fa-trash"></span>&nbsp;Please enter the IR Number you want to delete</h4>
      </div>
      <div class="modal-body">
      <form id="delete_ir" method="POST"> 
      <div class="form-group">
                <label>Select IR Number</label>
                  <?php $query = "select DISTINCT(IR_NO) from ir where IR_STATUS ='' ";
                $result2 = mysqli_query($link,$query);
...
<!--########################################END MODAL ###############################################################-->
        ...
        <div class="col-xs-12">
          ...
          <div class="box box-primary color-palette-box">
            <div class="box-header with-border ">
      	   <h3 class="box-title"><i class="fa fa-list-alt text-primary">&nbsp;</i>IR List</h3>
           
           <div class="btn-group pull-right">
           <a class="btn btn-success btn-sm" data-toggle="modal" data-target="#myModal"><span class="fa fa-edit"></span>&nbsp;<strong>Edit</strong></a>
           <a class="btn btn-default btn-sm " data-toggle="modal" data-target="#myModal2"><span class="fa fa-trash"></span>&nbsp;<strong>Delete IR</strong></a>
           </div>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
  ...
  <!--
             <table id="example1" class="datatable table table-bordered table-striped table-condensed" >
                <thead>
        <tr>
          ...
          <th>Date Modified</th>
                ...
                <tbody>
                  <?php
$query = "select * from ir inner join customer ON ir.CUSTOMER_ID = customer.CUSTOMER_ID where ir.IR_STATUS =''";
...
$IR_DATECREATED = $row['IR_DATECREATED'];
$IR_DATEMODIFIED = $row['IR_DATEMODIFIED'];
?>
   <tr>
<td class="text-center"><a  class="btn btn-sm btn-danger" href="javascript:delete_id('<?php echo $IR_ID; ?>')" title="Inactive Item"><span class="fa fa-trash-o"></span></a></td>             
<td><?php echo $DR_NO; ?></td>
<td><?php echo $IR_NO; ?></td>
<td><?php echo $CUSTOMER_NAME; ?></td>
<td><?php echo $CYLINDER_ID; ?></td>
<td><?php echo $IR_RETURNDATE; ?></td>
<td><?php echo $IR_DATECREATED; ?></td>
<td><?php echo $IR_DATEMODIFIED; ?></td>
